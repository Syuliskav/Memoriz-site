        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {!isCorrect && (
                <span className="text-xs px-2 py-0.5 rounded font-mono font-medium bg-white/80 dark:bg-slate-900 border border-slate-200 dark:border-slate-700">
                  Gabarito: ({deducedAnswer})
                </span>
              )}
            </div>
            <p className="text-xs opacity-85 mt-0.5">
              {isCorrect
                ? `Você marcou (${selectedLetter}). +15 XP acumulados!`
                : `Você marcou (${selectedLetter}). Alternativa correta: (${deducedAnswer}).`}
            </p>
          </div>
        </div>
      </div>
        ...
        {editingNote ? (
          <div className="space-y-2">
            <textarea
              ...
              className="w-full p-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md text-xs text-slate-800 dark:text-slate-200 focus:outline-none focus:border-indigo-500"
              rows={3}
            />
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                className="flex items-center gap-1 px-3 py-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded text-xs font-medium"
              >
          ...
          <div className="p-3 bg-amber-50/50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/40 rounded text-xs text-amber-950 dark:text-amber-200 font-serif">
            {note}
          </div>
        ) : (
