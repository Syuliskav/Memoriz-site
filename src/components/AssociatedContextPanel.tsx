...
export const AssociatedContextPanel: React.FC<AssociatedContextPanelProps> = ({ context }) => {
  ...
  return (
    <div className="h-full flex flex-col bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-xs">
      {/* Header Bar with Reading Controls */}
      <div className="px-4 py-2.5 bg-slate-50 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-700/80 flex items-center justify-between gap-2">
        ...
        {/* Font & Zoom Controls */}
        <div className="flex items-center gap-1 bg-white dark:bg-slate-900 px-2 py-0.5 rounded-md border border-slate-200 dark:border-slate-700">
          <button
            onClick={() => setFontFamily(f => (f === 'serif' ? 'sans' : 'serif'))}
            className="px-1.5 py-0.5 text-xs font-medium rounded text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer"
          ...
          <span className="text-[11px] font-mono text-slate-600 dark:text-slate-400 w-4 text-center">{fontSize}</span>
          <button
      ...
      <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-3">
        {context.title && (
          <h3 className="font-semibold text-base sm:text-lg text-slate-900 dark:text-slate-100 leading-snug">
            {context.title}
          </h3>
        )}

        {context.source && (
          <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 italic pb-2 border-b border-slate-100 dark:border-slate-800">
            {cleanUrl ? (
              <div className="flex items-center gap-1.5">
                <a
                  ...
                  className="p-1 -ml-1 rounded-md text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-200 hover:bg-indigo-50 dark:hover:bg-indigo-950/50 transition-colors cursor-pointer shrink-0 inline-flex items-center justify-center"
                  title="Abrir link original da fonte em nova aba"
                >
                  <ExternalLink className="w-3.5 h-3.5" />
                </a>
                <span className="select-text text-slate-500 dark:text-slate-400">Fonte: {context.source}</span>
              </div>
            ) : (
